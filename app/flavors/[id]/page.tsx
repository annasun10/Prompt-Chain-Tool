...
export default async function FlavorDetailPage({
  ...
  async function createStep(formData: FormData) {
    ...
    const llm_system_prompt = String(formData.get("llm_system_prompt") ?? "").trim();
    const llm_user_prompt = String(formData.get("llm_user_prompt") ?? "").trim();
    const llm_temperature = Number(formData.get("llm_temperature") ?? 0.7);

    // REQUIRED DEFAULTS (you can change later)
    const llm_input_type_id = 1;
    const llm_output_type_id = 1;
    const llm_model_id = 1;
    const humor_flavor_step_type_id = 1;
  ...
  async function updateStep(formData: FormData) {
    ...
    if (!stepId || !flavorId || !description) {
      return;
    }
    ...
    <main className="min-h-screen bg-white px-6 py-10 dark:bg-black">
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Slug:{" "}
              <span className="rounded bg-gray-100 px-2 py-1 font-mono dark:bg-gray-900">
                {flavor.slug || "no-slug"}
              </span>
            </p>
          </div>
        ...
        <section className="rounded-2xl border border-gray-200 bg-gray-50 p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="mb-4">
            <h2 className="text-xl font-semibold">Create a New Step</h2>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-semibold">Existing Steps</h2>
